...
{
    internal class FileStorageCrud1N : CRUD1NBase<DTO.FileStorage, TblFileStorage, int>
    {
        protected override int GetLastCreatedId() { return _lastCreatedItem.Id; }
        protected override void InsertIdInDTO(DTO.FileStorage dtoItem, int idValue) { dtoItem.Id = idValue; }
        protected override int GetIdFromDTO(DTO.FileStorage dtoItem) { return dtoItem.Id; }
        protected override List<int> GetIdListFromDTOList(List<DTO.FileStorage> dtoList) { return dtoList.Select(x => x.Id).ToList(); }
